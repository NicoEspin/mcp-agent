...
export class PlaywrightService implements OnModuleInit, OnModuleDestroy {
  ...
  private async isSessionValid(session: BrowserSession): Promise<boolean> {
    try {
  ...
  private async cleanupSession(sessionId: SessionId): Promise<void> {
    ...
    if (session) {
      try {
        ...
        this.logger.warn(`Error cleaning up session ${sessionId}: ${error.message}`);
      }
  ...
  async navigate(
    ...
    try {
      ...
      if (error.message.includes('Target page, context or browser has been closed')) {
        this.logger.warn(`Session ${sessionId} was closed during navigation, cleaning up and retrying once`);
        await this.cleanupSession(sessionId);
        ...
        this.logger.debug(`Navigated to ${url} (session: ${sessionId}) after recovery`);
      } else {
        this.logger.error(`Navigation failed for session ${sessionId}: ${error.message}`);
        throw error;
      }
    }
  }
  ...
  async takeScreenshot(
    ...
    try {
      ...
      if (error.message.includes('Target page, context or browser has been closed')) {
        this.logger.warn(`Session ${sessionId} was closed during screenshot, cleaning up and retrying once`);
        await this.cleanupSession(sessionId);
        ...
        try {
          ...
          this.logger.error(`Failed to take screenshot after session recovery for ${sessionId}: ${retryError.message}`);
          throw new Error(`Screenshot failed for session ${sessionId} after recovery attempt: ${retryError.message}`);
        }
      }
